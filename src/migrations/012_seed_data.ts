...
export const up = async (conn: mysql.PoolConnection): Promise<void> => {
  ...
  for (const pm of paymentMethods) {
    await conn.execute(
      ...
      [uuidv7(), pm.name, pm.type, pm.code, pm.admin_fee, pm.admin_fee_percent],
    );
  }

  const categories = ["Konser", "Festival", "Seminar", "Olahraga", "Pameran"];
  for (const cat of categories) {
    await conn.execute(`INSERT INTO categories (id, name) VALUES (?, ?)`, [
      uuidv7(),
      cat,
    ]);
  }
  ...
  // generate password hash for "admin123" using bcrypt with 10 salt rounds
